...
async def get_file_content(request: GitFileContentRequest = Body(...)):
    ...
    """
    try:
        return git_service.get_file_content_at_version(request.repo_path, request.file_path, request.version)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def batch_commit(request: GitBatchCommitRequest = Body(...)):
    ...
    """
    try:
        return git_service.batch_commit(request.repo_path, request.file_groups, request.message_template)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
