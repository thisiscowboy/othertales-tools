#!/usr/bin/env python3
"""
Codebase Export Utility

This script exports all code files from a codebase into a single text file.
It provides options for excluding certain directories and files, selecting 
specific file types, and formatting the output.
"""
...
def parse_arguments():
    ...
    parser.add_argument('-e', '--extensions', type=str, default='.py,.js,.java,.cpp,.h,.html,.css,.md,.txt',
                        help='Comma-separated list of file extensions to include (default: common code files)')
    
    parser.add_argument('-x', '--exclude', type=str, default='node_modules,venv,.git,__pycache__,*.pyc,*.pyo,*.pyd,*.so,*.dll,*.exe',
    ...
    parser.add_argument('--include-line-numbers', action='store_true',
                        help='Include line numbers in the output')
    
    parser.add_argument('--toc', action='store_true',
                        help='Generate a table of contents at the beginning')
...
def scan_directory(root_dir, files_list, args):
    ...
    for path in root_path.rglob('*'):
        if any(exclude_dir in str(path.relative_to(root_path)) for exclude_dir in exclude_dirs if exclude_dir):
            continue
...
def export_codebase(args):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        # Write header
        header = args.header.replace('{timestamp}', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        f.write(header + "\n\n")
